...
C     SPECIFICATIONS:
C     -----------------------------------------------------------------C
      USE GLOBAL,      ONLY:IOUT,NCOL,NROW,NLAY,IUNSTR,NODES
...
C-------READ PARAMETER WITHOUT INSTANCES
     ...
     &                      IOUT,NCOL,NROW,NLAY,IRDFLG,IUNSTR,NODES)
          ELSE
C-------READ INSTANCES
            NINLST=NLST/NUMINST
            DO 10 I=1,NUMINST
...
C3A-----IF THERE ARE NEW NON-PARAMETER STREAM CELLS, READ THEM
   ...
   99          FORMAT(1X,/1X,'THE NUMBER OF ACTIVE STREAM CELLS (',
     ...
     1             NROW,NLAY,IRDFLG,IUNSTR,NODES)
         END IF
      ELSE
C
...
C29-----SAVE STREAMFLOWS OUT OF EACH REACH ON DISK.                    C
      DO 615 L=1,NSTREM
      ND=ISTRM(1,L)
      IF(IBOUND(ND).LE.0) GO TO 615
      BUFF(ND)=BUFF(ND)+STRM(9,L)
  615 CONTINUE
      IF(IUNSTR.EQ.0)THEN
        CALL UBUDSV(KSTP,KPER,STRTXT,ISTCB2,BUFF,NCOL,NROW,NLAY,IOUT)
      ELSE
        CALL UBUDSVU(KSTP,KPER,STRTXT,ISTCB2,BUFF,NODES,IOUT,
     1    PERTIM,TOTIM)
      ENDIF
C                                                                      C
...
C31-----RETURN.                                                        C
     ...
     1          IOUT,NCOL,NROW,NLAY,IRDFLG,IUNSTR,NODES)
C     *****************************************************************C
...
C5------READ AND PRINT DATA FOR EACH STREAM CELL.
    ...
    4 FORMAT(/,4X,'LAYER   ROW    COL    SEGMENT   REACH   STREAMFLOW',
     ...
     3'ELEVATION      ELEVATION',/3X,110('-'))
      ELSE
       WRITE(IOUT,14)
   14 FORMAT(/,9X,'NODE    SEGMENT   REACH   STREAMFLOW',
     16X,'STREAM    STREAMBED     STREAMBED BOT  STREAMBED TOP',/17X,
     2'NUMBER   NUMBER                   STAGE   CONDUCTANCE',6X,
     3'ELEVATION      ELEVATION',/3X,110('-'))
      ENDIF
      ENDIF
...
C  Check for illegal grid location
  ...
  250 CONTINUE
      ...
      DO 251 II=LSTBEG,N
      READ(IN,7)NOD,ISTRM(4,II),ISTRM(5,II),STRM(1,II),STRM(2,II),
     1STRM(3,II),STRM(4,II),STRM(5,II)
    7 FORMAT(3I10,5F10.0)
      IF(IRDFLG.EQ.0) WRITE(IOUT,8)NOD,ISTRM(4,II),
     1ISTRM(5,II),STRM(1,II),STRM(2,II),STRM(3,II),STRM(4,II),STRM(5,II)
    8 FORMAT(1X,1X,I9,2I9,7X,G11.4,G12.4,G11.4,4X,2G13.4)
      ISTRM(1,II)=NOD
      ISTRM(2,II)=1
      ISTRM(3,II)=1
C
