...
c
     ...
     [           iblck, id, kk, nred, ierror
      ...
      integer, dimension(:), allocatable :: lrowptr, list
...
c      new fill-in caused by red node elimination -> level = 1
          ...
          if (icolour(jafwk(ii)) < 0) lrowptr( jafwk(ii) ) = 1
        enddo
     ...
     [               nblack,
     [               first, level, levptr, lrowptr, size(levptr))
...
c     the size of jafwk is bigger than njaf...
      ...
      deallocate(levptr, lrowptr, list, jafwk0)
...
c
     ...
     [        maxint, iblck, id, kk, idk, idkk
      ...
      integer, dimension(:), allocatable :: list, lrowptr
...
c     lsize = njaf+nblack
      ...
      allocate( list(nblack), lrowptr(nblack), stat = ierror )
      if (ierror /= 0) stop "== not enough memory (xmdsfacd) =="
...
c      new fill-in caused by red node elimination -> level = 1
     ...
     [               n, first, level, levptr, lrowptr, size(levptr))
...
c     afwk0 => afwk
      ...
      deallocate(row, list, levptr, lrowptr, jafwk0, afwk0)
...
c
     ...
     [        first, level, levptr(nlevptr), lrowptr(n), nlevptr
c
c     local 
c
...
c
  ...
  100     continue
          ilevel1 = levptr(ii)+lrowptr(irow)+1
          ilevel2 = lrowptr(jaf(ii))
...
c     of each row appear as the first element of each row
c
c     and sort row entries in increasing column order.  ?????????????
c
c
c
...
c
     ...
     [        first, level, levptr(nlevptr), lrowptr(nblack), nblack
c
c     local 
c
