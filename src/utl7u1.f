...
C5------Return.
      RETURN
      END
      SUBROUTINE U1DREL(A,ANAME,JJ,K,IN,IOUT)
C     ******************************************************************
...
C8------CONTROL RECORD ERROR.
  ...
  502    FORMAT(1X,/1X,'ERROR READING ARRAY CONTROL RECORD FOR ',A,':')
      ...
      CALL USTOP(' ')
      END
      SUBROUTINE U1DINT(IA,ANAME,JJ,K,IN,IOUT)
C     ******************************************************************
C     ROUTINE TO INPUT 1-D REAL DATA MATRICES
C       A IS ARRAY TO INPUT
C       ANAME IS 24 CHARACTER DESCRIPTION OF A
...
C8------CONTROL RECORD ERROR.
  ...
  502    FORMAT(1X,/1X,'ERROR READING ARRAY CONTROL RECORD FOR ',A,':')
      ...
      CALL USTOP(' ')
      END
      SUBROUTINE U2DREL8(A,ANAME,II,JJ,K,IN,IOUT)
C     ******************************************************************
C     ROUTINE TO INPUT 2-D REAL DATA MATRICES
C       A IS ARRAY TO INPUT
C       ANAME IS 24 CHARACTER DESCRIPTION OF A
...
C
C6------IF PRINT CODE (IPRN) >0 OR =0 THEN PRINT ARRAY VALUES.
  320 IF(IPRN.GE.0) CALL ULAPRW8(A,ANAME,0,0,JJ,II,0,IPRN,IOUT)
C
C7------RETURN
      RETURN
C
...
C5------RETURN
      RETURN
      END
      SUBROUTINE ULAPRW8(BUF,TEXT,KSTP,KPER,NCOL,NROW,ILAY,IPRN,IOUT)
C     ******************************************************************
C     PRINT 1 LAYER ARRAY
C     ******************************************************************
C
...
C1------INFORMATION.
      WRITE(ICHN) KSTP,KPER,PERTIMS,TOTIMS,TEXT,NCOL,NROW,ILAY
C
...
C     ------------------------------------------------------------------
      ...
      DOUBLE PRECISION PERTIM,TOTIM,DELT
      REAL DT,PTIM,TTIM
      REAL, SAVE, DIMENSION(:,:),ALLOCATABLE  ::BUF2D
...
C1------WRITE TWO UNFORMATTED RECORDS IDENTIFYING DATA.
    ...
    1 FORMAT(1X,'UBDSV3  SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I7,', STRESS PERIOD',I7)
      ...
      WRITE(IBDCHN) IMETH,DT,PTIM,TTIM
C
...
C2B-----LAYER NUMBERS;  SECOND RECORD CONTAINS FLOW VALUES.
         ALLOCATE(BUF2D(NCOL,NROW))
         WRITE(IBDCHN) ((IBUFF(J,I),J=1,NCOL),I=1,NROW)
         DO I=1,NROW
         DO J=1,NCOL
         N = IBUFF(J,I)
         KLAYER = (N-1) / (NCOL*NROW) + 1
         BUF2D(J,I) = BUFF(J,I,KLAYER)
         ENDDO
         ENDDO
         WRITE(IBDCHN) ((BUF2D(J,I),J=1,NCOL),I=1,NROW)
         DEALLOCATE (BUF2D)
      END IF
C
C3------RETURN
...
C1------WRITE UNFORMATTED RECORDS IDENTIFYING DATA.
      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV4U SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I7,', STRESS PERIOD',I7)
      WRITE(IBDCHN) KSTP,KPER,TEXT,NODES,1,-1
      WRITE(IBDCHN) 5,DT,PTIM,TTIM
      WRITE(IBDCHN) NAUX+1
      IF(NAUX.GT.0) WRITE(IBDCHN) (AUXTXT(N),N=1,NAUX)
      WRITE(IBDCHN) NLIST
...
C     ------------------------------------------------------------------
   10 FORMAT(1X,A)
      ...
      END
      SUBROUTINE ULAPRU(BUF,TEXT,KSTP,KPER,NSTRT,NNDLAY,ILAY,IPRN,IOUT,
     *  PERTIM,TOTIM,NODES)
C     ******************************************************************
C     PRINT 1 LAYER ARRAY IN UNSTRUCTURED FORMAT
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C1------PRINT A HEADER DEPENDING ON ILAY
      IF(ILAY.GT.0) THEN
         WRITE(IOUT,1) TEXT,ILAY,KSTP,KPER,PERTIM,TOTIM
    1    FORMAT('1',/2X,A,' IN LAYER ',I7,' AT END OF TIME STEP ',I7,
     1     ' IN STRESS PERIOD ',I7,' STRESS TIME',G15.6,' TOTAL TIME',
     2     G15.6/2X,137('-'))
      ELSE IF(ILAY.LT.0) THEN
         WRITE(IOUT,2) TEXT,KSTP,KPER,PERTIM,TOTIM
    2    FORMAT('1',/1X,A,' FOR CROSS SECTION AT END OF TIME STEP',I7,
     1     ' IN STRESS PERIOD ',I7,' STRESS TIME',G15.6,' TOTAL TIME',
     2       G15.6,/1X,105('-'))
      END IF
C
...
C
C------------FORMAT 11G10.3
   10 WRITE(IOUT,11) J1,(BUF(J),J=J1,J2)
   11 FORMAT(1X,I8,'..',2X,1PG10.3,10(1X,G10.3))
      GO TO 1000
C
C------------FORMAT 9G13.6
   20 WRITE(IOUT,21) J1,(BUF(J),J=J1,J2)
   21 FORMAT(1X,I8,'..',2X,1PG13.6,8(1X,G13.6))
      GO TO 1000
C
C------------FORMAT 15F7.1
   30 WRITE(IOUT,31) J1,(BUF(J),J=J1,J2)
   31 FORMAT(1X,I8,'..',1X,15(1X,F7.1))
      GO TO 1000
C
C------------FORMAT 15F7.2
   40 WRITE(IOUT,41) J1,(BUF(J),J=J1,J2)
   41 FORMAT(1X,I8,'..',1X,15(1X,F7.2))
      GO TO 1000
C
C------------FORMAT 15F7.3
   50 WRITE(IOUT,51) J1,(BUF(J),J=J1,J2)
   51 FORMAT(1X,I8,'..',1X,15(1X,F7.3))
      GO TO 1000
C
C------------FORMAT 15F7.4
   60 WRITE(IOUT,61) J1,(BUF(J),J=J1,J2)
   61 FORMAT(1X,I8,'..',1X,15(1X,F7.4))
      GO TO 1000
C
C------------FORMAT 20F5.0
   70 WRITE(IOUT,71) J1,(BUF(J),J=J1,J2)
   71 FORMAT(1X,I8,'..',1X,20(1X,F5.0))
      GO TO 1000
C
C------------FORMAT 20F5.1
   80 WRITE(IOUT,81) J1,(BUF(J),J=J1,J2)
   81 FORMAT(1X,I8,'..',1X,20(1X,F5.1))
      GO TO 1000
C
C------------FORMAT 20F5.2
   90 WRITE(IOUT,91) J1,(BUF(J),J=J1,J2)
   91 FORMAT(1X,I8,'..',1X,20(1X,F5.2))
      GO TO 1000
C
C------------FORMAT 20F5.3
  100 WRITE(IOUT,101) J1,(BUF(J),J=J1,J2)
  101 FORMAT(1X,I8,'..',1X,20(1X,F5.3))
      GO TO 1000
C
C------------FORMAT 20F5.4
  110 WRITE(IOUT,111) J1,(BUF(J),J=J1,J2)
  111 FORMAT(1X,I8,'..',1X,20(1X,F5.4))
      GO TO 1000
C
C------------FORMAT 10G11.4
  120 WRITE(IOUT,121) J1,(BUF(J),J=J1,J2)
  121 FORMAT(1X,I8,'..',2X,1PG11.4,9(1X,G11.4))
      GO TO 1000
C
C------------FORMAT 10F6.0
  130 WRITE(IOUT,131) J1,(BUF(J),J=J1,J2)
  131 FORMAT(1X,I8,'..',1X,10(1X,F6.0))
      GO TO 1000
C
C------------FORMAT 10F6.1
  140 WRITE(IOUT,141) J1,(BUF(J),J=J1,J2)
  141 FORMAT(1X,I8,'..',1X,10(1X,F6.1))
      GO TO 1000
C
C------------FORMAT 10F6.2
  150 WRITE(IOUT,151) J1,(BUF(J),J=J1,J2)
  151 FORMAT(1X,I8,'..',1X,10(1X,F6.2))
      GO TO 1000
C
C------------FORMAT 10F6.3
  160 WRITE(IOUT,161) J1,(BUF(J),J=J1,J2)
  161 FORMAT(1X,I8,'..',1X,10(1X,F6.3))
      GO TO 1000
C
C------------FORMAT 10F6.4
  170 WRITE(IOUT,171) J1,(BUF(J),J=J1,J2)
  171 FORMAT(1X,I8,'..',1X,10(1X,F6.4))
      GO TO 1000
C
C------------FORMAT 10F6.5
  180 WRITE(IOUT,181) J1,(BUF(J),J=J1,J2)
  181 FORMAT(1X,I8,'..',1X,10(1X,F6.5))
      GO TO 1000
C
C------------FORMAT 5G12.5
  190 WRITE(IOUT,191) J1,(BUF(J),J=J1,J2)
  191 FORMAT(1X,I8,'..',1X,1PG12.5,4(1X,G12.5))
      GO TO 1000
C
C------------FORMAT 6G11.4
  200 WRITE(IOUT,201) J1,(BUF(J),J=J1,J2)
  201 FORMAT(1X,I8,'..',1X,1PG11.4,5(1X,G11.4))
      GO TO 1000
C
C------------FORMAT 7G9.2
  210 WRITE(IOUT,211) J1,(BUF(J),J=J1,J2)
  211 FORMAT(1X,I8,'..',1X,1PG9.2,6(1X,G9.2))
...
C5------RETURN
     ...
     1                   NNDLAY,ILAY,ICHN,NODES)
C     ******************************************************************
C     SAVE 1 LAYER ARRAY ON DISK FOR UNSTRUCTURED FORMAT
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C1------WRITE AN UNFORMATTED RECORD CONTAINING IDENTIFYING INFORMATION.
      WRITE(ICHN) KSTP,KPER,PERTIMS,TOTIMS,TEXT,NSTRT,NNDLAY,ILAY
C
C2------WRITE AN UNFORMATTED RECORD CONTAINING ARRAY VALUES FOR THE LAYER
      WRITE(ICHN) (BUF(I),I=NSTRT,NNDLAY)
C
C3------RETURN
      RETURN
      END
      SUBROUTINE ULASV2U(BUFF,TEXT,KSTP,KPER,PERTIM,TOTIM,NSTRT,
     1                   NNDLAY,ILAY,ICHN,FMTOUT,LBLSAV,IBOUND,NODES)
C     ******************************************************************
C     SAVE 1 LAYER ARRAY ON DISK USING FORMATTED OUTPUT FOR UNSTRUCTURED FORMAT
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      CHARACTER*16 TEXT
      DOUBLE PRECISION PERTIM,TOTIM
      DIMENSION BUFF(NODES),IBOUND(NODES)
      CHARACTER*20 FMTOUT
C     ------------------------------------------------------------------
C
C1------WRITE A LABEL IF LBLSAV IS NOT 0.
      IF(LBLSAV.NE.0) WRITE(ICHN,5) KSTP,KPER,PERTIM,TOTIM,TEXT,NSTRT,
     1                 NNDLAY,ILAY,FMTOUT
5     FORMAT(1X,2I5,1P,2E15.6,1X,A,3I6,1X,A)
C
C2------WRITE THE ARRAY USING THE SPECIFIED FORMAT.
      WRITE(ICHN,FMTOUT) (BUFF(I),I=NSTRT,NNDLAY)
C
C3------RETURN
      RETURN
      END
      SUBROUTINE ULASV3U(IDATA,TEXT,KSTP,KPER,PERTIM,TOTIM,NSTRT,
     1                   NNDLAY,ILAY,ICHN,FMTOUT,LBLSAV,NODES)
C     ******************************************************************
C     SAVE 2-D (LAYER) INTEGER ARRAY ON DISK USING FORMATTED OUTPUT FOR UNSTRUCTURED FORMAT
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C
C2------WRITE THE ARRAY USING THE SPECIFIED FORMAT.
      WRITE(ICHN,FMTOUT) (IDATA(I),I=NSTRT,NNDLAY)
C
C3------RETURN.
      RETURN
      END
...
C1------WRITE AN UNFORMATTED RECORD IDENTIFYING DATA.
      WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBUDSVU SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I7,', STRESS PERIOD',I7)
      WRITE(IBDCHN) KSTP,KPER,TEXT,NJA,1,1
C
C2------WRITE AN UNFORMATTED RECORD CONTAINING VALUES FOR
C2------EACH CELL IN THE GRID.
      WRITE(IBDCHN) BUFF
...
C3------RETURN
      RETURN
      END
      SUBROUTINE UBDSV1U(KSTP,KPER,TEXT,IBDCHN,BUFF,NJA,IOUT,
...
C1------WRITE TWO UNFORMATTED RECORDS IDENTIFYING DATA.
      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV1U SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I7,', STRESS PERIOD',I7)
      PTIM=PERTIM
      TTIM=TOTIM
      DT=DELT
...
C3------RETURN
      RETURN
      END
      SUBROUTINE UBDSV2U(KSTP,KPER,TEXT,IBDCHN,NODES,
     1          NLIST,IOUT,DELT,PERTIM,TOTIM,IBOUND)
C     ******************************************************************
C     WRITE HEADER RECORDS FOR CELL-BY-CELL FLOW TERMS FOR ONE COMPONENT
C     OF FLOW USING A LIST STRUCTURE.  EACH ITEM IN THE LIST IS WRITTEN
...
C1------WRITE THREE UNFORMATTED RECORDS IDENTIFYING DATA.
      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV2U SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I7,', STRESS PERIOD',I7)
      PTIM=PERTIM
      TTIM=TOTIM
      DT=DELT
...
C2------RETURN
      RETURN
      END
      SUBROUTINE UBDSVAU(IBDCHN,NODES,N,Q,IBOUND)
C     ******************************************************************
C     WRITE ONE VALUE OF CELL-BY-CELL FLOW USING A LIST STRUCTURE.
C     ******************************************************************
C
...
C3------RETURN
      RETURN
      END
      SUBROUTINE UBDSV3U(KSTP,KPER,TEXT,IBDCHN,BUFF,IBUFF,IDIM,NOPT,
...
C1------WRITE TWO UNFORMATTED RECORDS IDENTIFYING DATA.
      IF(IOUT.GT.0) WRITE(IOUT,1) TEXT,IBDCHN,KSTP,KPER
    1 FORMAT(1X,'UBDSV3U SAVING "',A16,'" ON UNIT',I4,
     1     ' AT TIME STEP',I7,', STRESS PERIOD',I7)
      PTIM=PERTIM
      TTIM=TOTIM
      DT=DELT
      WRITE(IBDCHN) KSTP,KPER,TEXT,IDIM,1,-1
      IMETH=3
      IF(NOPT.EQ.1) IMETH=4
      WRITE(IBDCHN) IMETH,DT,PTIM,TTIM
C
...
C2A-----FOR THE NODES IN BUFF.
         WRITE(IBDCHN) (BUFF(IBUFF(IDI)),IDI = 1,IDIM)
      ELSE
C2B-----WRITE TWO RECORDS WHEN NOPT IS NOT 1.  FIRST RECORD CONTAINS
C2B-----NODE NUMBERS;  SECOND RECORD CONTAINS FLOW VALUES.
         WRITE(IBDCHN) (IBUFF(IDI),IDI=1,IDIM)
         WRITE(IBDCHN) (BUFF(IBUFF(IDI)),IDI = 1,IDIM)
      END IF
C
C3------RETURN
...
C3------RETURN
      ...
      SUBROUTINE UBDSVHDR(IUNSTR,KSTP,KPER,IOUT,IBNDCB,ICLNCB,NODES,
     1  NCLNNDS,NCOL,NROW,NLAY,NBND,NBNDVL,NAUX,IBOUND,
     2  TEXT,BNDAUX,DELT,PERTIM,TOTIM,BND)
C     ******************************************************************
C     WRITE THE COMPACT BUDGET HEADER FOR EITHER STRUCTURED OR
...
C3A------WRITE STRUCTURED HEADER
          CALL UBDSV4(KSTP,KPER,TEXT,NAUX,BNDAUX,IBNDCB,NCOL,NROW,
     1                NLAY,NBNDGW,IOUT,DELT,PERTIM,TOTIM,IBOUND)
        ELSE 
C
C3A------WRITE UNSTRUCTURED HEADER
          CALL UBDSV4U(KSTP,KPER,TEXT,NAUX,BNDAUX,IBNDCB,NODES,
     1                 NBNDGW,IOUT,DELT,PERTIM,TOTIM,IBOUND)
        ENDIF
C
C4------WRITE CLN BUDGET FILE HEADER
        IF(ICLNCB.GT.0)
     1    CALL UBDSV4U(KSTP,KPER,TEXT,NAUX,BNDAUX,ICLNCB,NCLNNDS,
...
C5------RETURN
      ...
      SUBROUTINE UBDSVREC(IUNSTR,N,NODES,NCLNNDS,IBNDCB,ICLNCB,NBNDVL,
     1  LAUX,NAUX,Q,VAL,IBOUND,NCOL,NROW,NLAY)
C     ******************************************************************
C     SAVE A SINGLE RECORD IN THE COMPACT BUDGET FORM.  USE THE NODE
...
C     ------------------------------------------------------------------
C
C1------IF CLN NODE, WRITE TO CLN BUDGET     
        IF(N.GT.NODES) THEN
          IF(ICLNCB.GT.0) THEN
            CALL UBDSVBU(ICLNCB,NCLNNDS,N-NODES,Q,VAL,NBNDVL,NAUX,
     1                 LAUX,IBOUND)
          ENDIF
        ELSE
C
C2------WRITE STRUCTURED GW RECORD
          IF(IUNSTR.EQ.0)THEN
            IL = (N-1) / (NCOL*NROW) + 1
            IJ = N - (IL-1)*NCOL*NROW
            IR = (IJ-1)/NCOL + 1
            IC = IJ - (IR-1)*NCOL
            CALL UBDSVB(IBNDCB,NCOL,NROW,IC,IR,IL,Q,
     1                  VAL,NBNDVL,NAUX,LAUX,IBOUND,NLAY)
          ELSE
C
C3------WRITE UNSTRUCTURED GW RECORD     
            CALL UBDSVBU(IBNDCB,NODES,N,Q,VAL,NBNDVL,NAUX,LAUX,IBOUND)
          ENDIF
        ENDIF
C
C4------RETURN
