...
C4------PRINT # OF SPECIFIED-HEAD CELLS THIS STRESS PERIOD
      WRITE(IOUT,2) NCHDS
    2 FORMAT(1X,//1X,I6,' TIME-VARIANT SPECIFIED-HEAD CELLS')
C
C5------SET CHDS(1,II) TO GLOBAL NODE NUMBER AND
...
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:IOUT,HNEW,HOLD,PERLEN,IUNSTR
...
C
 ...
 200    FORMAT(/,' ***WARNING***  FOR CHD CELL (',I3,',',I5,',',I5,
     ...
     &' (GWF2CHD7AD)',/)
C
 201    FORMAT(/,' ***WARNING***  FOR CHD CELL (',I9,
     &'), START HEAD AND END HEAD DIFFER',/,
     &' FOR A STRESS PERIOD OF ZERO LENGTH --',/,
     &' USING ENDING HEAD AS CONSTANT HEAD',
     &' (GWF2CHD7AD)',/)
...
C6------UPDATE THE APPROPRIATE HNEW VALUE
      HNEW(N)=HNEW(N)+HB
cSP      HOLD(N)=HNEW(N) 
CSP      HOLD SHOULD BE AS UPDATED WHEN ENTERING NEW TIME STEP AND NOT THIS INTERPOLATED VALUE
  100 CONTINUE
C
C7------RETURN
      RETURN
      END
      SUBROUTINE GWF2CHD7U1DA
